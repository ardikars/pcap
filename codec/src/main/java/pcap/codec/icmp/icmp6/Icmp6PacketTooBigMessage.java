...
public class Icmp6PacketTooBigMessage extends Icmp.IcmpTypeAndCode {
   ...
   * because the packet is larger than the MTU of the outgoing link. The information in this message
   * is used as part of the Path MTU Discovery process [PMTU].
   *
   * <p>Originating a Packet Too Big Message makes an exception to one of the rules as to when to
   * originate an Icmp6InverseNeighborDiscoverySolicitation error message. Unlike other messages, it
   * is sent in response to a packet received with an IPv6 multicast destination address, or with a
   * link-layer multicast or link-layer broadcast address.
   */
