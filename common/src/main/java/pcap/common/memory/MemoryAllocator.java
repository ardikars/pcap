...
public interface MemoryAllocator {
  ...
  static MemoryAllocator create(String name) {
    ...
    while (iterator.hasNext()) {
      ...
      if (service.name().equals(name) && !(service instanceof AbstractPooledMemoryAllocator)) {
        return service;
      }
    }
  ...
  static MemoryAllocator create(String name, int poolSize, int maxPoolSize, int maxCapacity) {
    ...
    while (iterator.hasNext()) {
      MemoryAllocator service = iterator.next();
      if (service.name().equals(name) && service instanceof AbstractPooledMemoryAllocator) {
        ((AbstractPooledMemoryAllocator) service).create(poolSize, maxPoolSize, maxCapacity);
        return service;
      }
    }
    throw new IllegalStateException("No memory allocator implementation for (" + name + ").");
  ...
  default Memory wrap(byte[] bytes) {
  ...
  default Memory wrap(ByteBuffer bb) {
  ...
  default Memory assemble(Memory... memories) {
  ...
  abstract class AbstractPooledMemoryAllocator implements MemoryAllocator {
