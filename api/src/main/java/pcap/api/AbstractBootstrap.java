...
abstract class AbstractBootstrap {
    ...
    protected Pcap openHandle() throws PcapErrorException, CanSetTStampTypeException, PermissionDeniedException, PromiscPermissionDeniedException, RfmonNotSupportedException, NoSuchDeviceException, TStampPrecisionNotSupportedException, InterfaceNotUpException, ActivatedException {
        synchronized (Pcap.LOCK) {
            if (offline) {
                ...
                if (timestampPrecision == null) {
                    ...
                    LOGGER.debug("Opening file ({}) with timestamp precision ({})", file, timestampPrecision.value());
                    pointer = Pcap.MAPPING.pcap_open_offline_with_tstamp_precision(Pcap.SCOPE.allocateCString(file),
                            timestampPrecision.value(), errbuf);
                }
                ...
                if (timestampType == null && timestampPrecision == null) {
                    return pcapInterface.openLive(snaplen == 0 ? 65535 : snaplen, promiscuous, timeout == 0 ? 2000 : timeout);
                } else if (timestampType == null || timestampPrecision == null) {
                    return pcapInterface.openLive();
                } else {
                    return pcapInterface.openLive(snaplen == 0 ? 65535 : snaplen, promiscuous, rfmon,
                            timeout == 0 ? 2000 : timeout, timestampType == null ? Timestamp.Type.HOST : timestampType,
                            immediateMode, bufferSize, timestampPrecision == null ? Timestamp.Precision.MICRO : timestampPrecision);
                }
            }
        }
    }
