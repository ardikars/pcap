...
public class PcapInterface implements Interface {
    ...
    public static Interface lookup() throws PcapErrorException {
        synchronized (Pcap.LOCK) {
            ...
            Interface pcapInterface = StreamSupport.stream(findAll().spliterator(), false)
    ...
    public Pcap openLive(int snaplen,
                         ...
                         int bufferSize,
                         Timestamp.Precision tstampPrecision) throws PcapErrorException, ActivatedException, CanSetTStampTypeException, NoSuchDeviceException, PermissionDeniedException, PromiscPermissionDeniedException, RfmonNotSupportedException, InterfaceNotUpException, TStampPrecisionNotSupportedException {
        synchronized (Pcap.LOCK) {
            Pointer<Byte> errbuf = Pcap.SCOPE.allocate(NativeTypes.INT8, Pcap.ERRBUF_SIZE);
